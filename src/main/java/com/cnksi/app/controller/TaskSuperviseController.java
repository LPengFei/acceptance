...
import com.cnksi.app.model.TaskSupervise; 
/**
 * 
 */
public class TaskSuperviseController extends KController {

 	public TaskSuperviseController(){
		super(TaskSupervise.class);
	} 
	...
	public void index() {
		...
		setAttr("page", TaskSupervise.me.paginate(queryParam));
	...
	public void edit() {
		...
		TaskSupervise record = null;
		if (idValue != null) {
			record = TaskSupervise.me.findById(idValue);
		}else{
			record = new TaskSupervise();
	...
	public void save() {
		TaskSupervise record = getModel(TaskSupervise.class, "record");
	...
	public void delete() {
		TaskSupervise record = TaskSupervise.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<TaskSupervise> p = TaskSupervise.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					TaskSupervise record = new TaskSupervise();
					record.put(map);
					record.save();
				}
			} else {
