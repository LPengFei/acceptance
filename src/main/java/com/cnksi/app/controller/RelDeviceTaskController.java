...
import com.cnksi.app.model.RelDeviceTask; 
/**
 * 
 */
public class RelDeviceTaskController extends KController {

 	public RelDeviceTaskController(){
		super(RelDeviceTask.class);
	} 
	...
	public void index() {
		...
		setAttr("page", RelDeviceTask.me.paginate(queryParam));
	...
	public void edit() {
		...
		RelDeviceTask record = null;
		if (idValue != null) {
			record = RelDeviceTask.me.findById(idValue);
		}else{
			record = new RelDeviceTask();
	...
	public void save() {
		RelDeviceTask record = getModel(RelDeviceTask.class, "record");
	...
	public void delete() {
		RelDeviceTask record = RelDeviceTask.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<RelDeviceTask> p = RelDeviceTask.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					RelDeviceTask record = new RelDeviceTask();
					record.put(map);
					record.save();
				}
			} else {
