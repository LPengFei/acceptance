 ...
 * 
 */
public class ResultSignnameController extends KController {

 	public ResultSignnameController(){
		super(ResultSignname.class);
	} 
	...
	public void index() {
		...
		setAttr("page", ResultSignname.me.paginate(queryParam));
	...
	public void edit() {
		...
		ResultSignname record = null;
		if (idValue != null) {
			record = ResultSignname.me.findById(idValue);
		}else{
			record = new ResultSignname();
	...
	public void save() {
		ResultSignname record = getModel(ResultSignname.class, "record");
	...
	public void delete() {
		ResultSignname record = ResultSignname.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<ResultSignname> p = ResultSignname.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					ResultSignname record = new ResultSignname();
					record.put(map);
					record.save();
				}
			} else {
