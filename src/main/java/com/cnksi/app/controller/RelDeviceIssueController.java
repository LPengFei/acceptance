...
import com.cnksi.app.model.RelDeviceIssue; 
/**
 * 
 */
public class RelDeviceIssueController extends KController {

 	public RelDeviceIssueController(){
		super(RelDeviceIssue.class);
	} 
	...
	public void index() {
		...
		setAttr("page", RelDeviceIssue.me.paginate(queryParam));
	...
	public void edit() {
		...
		RelDeviceIssue record = null;
		if (idValue != null) {
			record = RelDeviceIssue.me.findById(idValue);
		}else{
			record = new RelDeviceIssue();
	...
	public void save() {
		RelDeviceIssue record = getModel(RelDeviceIssue.class, "record");
	...
	public void delete() {
		RelDeviceIssue record = RelDeviceIssue.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<RelDeviceIssue> p = RelDeviceIssue.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					RelDeviceIssue record = new RelDeviceIssue();
					record.put(map);
					record.save();
				}
			} else {
