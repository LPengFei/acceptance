...
import com.cnksi.app.model.ProProjectScal; 
/**
 * 
 */
public class ProProjectScalController extends KController {

 	public ProProjectScalController(){
		super(ProProjectScal.class);
	} 
	...
	public void index() {
		...
		setAttr("page", ProProjectScal.me.paginate(queryParam));
	...
	public void edit() {
		...
		ProProjectScal record = null;
		if (idValue != null) {
			record = ProProjectScal.me.findById(idValue);
		}else{
			record = new ProProjectScal();
	...
	public void save() {
		ProProjectScal record = getModel(ProProjectScal.class, "record");
	...
	public void delete() {
		ProProjectScal record = ProProjectScal.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<ProProjectScal> p = ProProjectScal.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					ProProjectScal record = new ProProjectScal();
					record.put(map);
					record.save();
				}
			} else {
