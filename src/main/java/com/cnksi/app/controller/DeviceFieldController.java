...
import com.cnksi.app.model.DeviceField; 
/**
 * 
 */
public class DeviceFieldController extends KController {

 	public DeviceFieldController(){
		super(DeviceField.class);
	} 
	...
	public void index() {
		...
		setAttr("page", DeviceField.me.paginate(queryParam));
	...
	public void edit() {
		...
		DeviceField record = null;
		if (idValue != null) {
			record = DeviceField.me.findById(idValue);
		}else{
			record = new DeviceField();
	...
	public void save() {
		DeviceField record = getModel(DeviceField.class, "record");
	...
	public void delete() {
		DeviceField record = DeviceField.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<DeviceField> p = DeviceField.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					DeviceField record = new DeviceField();
					record.put(map);
					record.save();
				}
			} else {
