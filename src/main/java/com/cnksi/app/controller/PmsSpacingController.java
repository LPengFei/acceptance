 ...
 * 
 */
public class PmsSpacingController extends KController {

 	public PmsSpacingController(){
		super(PmsSpacing.class);
	} 
	...
	public void index() {
		...
		setAttr("page", PmsSpacing.me.paginate(queryParam));
	...
	public void edit() {
		...
		PmsSpacing record = null;
		if (idValue != null) {
			record = PmsSpacing.me.findById(idValue);
		}else{
			record = new PmsSpacing();
	...
	public void save() {
		PmsSpacing record = getModel(PmsSpacing.class, "record");
	...
	public void delete() {
		PmsSpacing record = PmsSpacing.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<PmsSpacing> p = PmsSpacing.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					PmsSpacing record = new PmsSpacing();
					record.put(map);
					record.save();
				}
			} else {
