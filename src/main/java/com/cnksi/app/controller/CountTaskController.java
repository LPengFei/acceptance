 ...
 * 
 */
public class CountTaskController extends KController {

 	public CountTaskController(){
		super(CountTask.class);
	} 
	...
	public void index() {
		...
		setAttr("page", CountTask.me.paginate(queryParam));
	...
	public void edit() {
		...
		CountTask record = null;
		if (idValue != null) {
			record = CountTask.me.findById(idValue);
		}else{
			record = new CountTask();
	...
	public void save() {
		CountTask record = getModel(CountTask.class, "record");
	...
	public void delete() {
		CountTask record = CountTask.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<CountTask> p = CountTask.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					CountTask record = new CountTask();
					record.put(map);
					record.save();
				}
			} else {
