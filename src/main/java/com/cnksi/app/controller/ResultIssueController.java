 ...
 * 
 */
public class ResultIssueController extends KController {

 	public ResultIssueController(){
		super(ResultIssue.class);
	} 
	...
	public void index() {
		...
		setAttr("page", ResultIssue.me.paginate(queryParam));
	...
	public void edit() {
		...
		ResultIssue record = null;
		if (idValue != null) {
			record = ResultIssue.me.findById(idValue);
		}else{
			record = new ResultIssue();
	...
	public void save() {
		ResultIssue record = getModel(ResultIssue.class, "record");
	...
	public void delete() {
		ResultIssue record = ResultIssue.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<ResultIssue> p = ResultIssue.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					ResultIssue record = new ResultIssue();
					record.put(map);
					record.save();
				}
			} else {
