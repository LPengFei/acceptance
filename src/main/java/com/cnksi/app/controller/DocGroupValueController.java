...
import com.cnksi.app.model.DocGroupValue; 
/**
 * 
 */
public class DocGroupValueController extends KController {

 	public DocGroupValueController(){
		super(DocGroupValue.class);
	} 
	...
	public void index() {
		...
		setAttr("page", DocGroupValue.me.paginate(queryParam));
	...
	public void edit() {
		...
		DocGroupValue record = null;
		if (idValue != null) {
			record = DocGroupValue.me.findById(idValue);
		}else{
			record = new DocGroupValue();
	...
	public void save() {
		DocGroupValue record = getModel(DocGroupValue.class, "record");
	...
	public void delete() {
		DocGroupValue record = DocGroupValue.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<DocGroupValue> p = DocGroupValue.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					DocGroupValue record = new DocGroupValue();
					record.put(map);
					record.save();
				}
			} else {
