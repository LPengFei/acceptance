 ...
 * 
 */
public class ResultItemController extends KController {

 	public ResultItemController(){
		super(ResultItem.class);
	} 
	...
	public void index() {
		...
		setAttr("page", ResultItem.me.paginate(queryParam));
	...
	public void edit() {
		...
		ResultItem record = null;
		if (idValue != null) {
			record = ResultItem.me.findById(idValue);
		}else{
			record = new ResultItem();
	...
	public void save() {
		ResultItem record = getModel(ResultItem.class, "record");
	...
	public void delete() {
		ResultItem record = ResultItem.me.findById(getPara());
		if (record != null) {
	...
	public void export()  throws IOException {
		KWebQueryVO queryParam = super.doIndex();
		Page<ResultItem> p = ResultItem.me.paginate(queryParam);
		...
		try {
			...
			if (!result.isVerfiyFail()) {
				for (Map<String, Object> map : result.getList()) {
					ResultItem record = new ResultItem();
					record.put(map);
					record.save();
				}
			} else {
