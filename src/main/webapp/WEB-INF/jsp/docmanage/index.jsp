...
<%--<div class="bjui-pageHeader">--%>
    ...
    <%--<form id="pagerForm" data-toggle="ajaxsearch" action="${ctx}/${appid }/${modelName}" method="post">--%>
        ...
        <%--<div class="bjui-searchBar ess-searchBar">--%>
            ...
            <%--&lt;%&ndash;<c:if test="${!empty(exports) or !empty(imports) }">&ndash;%&gt;--%>
            <%--&lt;%&ndash;<div class="btn-group">&ndash;%&gt;--%>
            <%--&lt;%&ndash;<button type="button" class="btn-default dropdown-toggle" data-toggle="dropdown"&ndash;%&gt;--%>
            <%--&lt;%&ndash;data-icon="copy">导出-批量操作<span class="caret"></span></button>&ndash;%&gt;--%>
            <%--&lt;%&ndash;<ul class="dropdown-menu right" role="menu">&ndash;%&gt;--%>
            <%--&lt;%&ndash;<c:forEach items="${exports }" var="e">&ndash;%&gt;--%>
            <%--&lt;%&ndash;<li><a href="${ctx}/${appid }/${modelName}/export?xlsid=${e.ieid}"&ndash;%&gt;--%>
            <%--&lt;%&ndash;data-toggle="doexport" data-confirm-msg="确定要${e.iename }吗？"&ndash;%&gt;--%>
            <%--&lt;%&ndash;class="blue">${e.iename }-导出</a></li>&ndash;%&gt;--%>
            <%--&lt;%&ndash;</c:forEach>&ndash;%&gt;--%>
            <%--&lt;%&ndash;<c:if test="${!empty(imports) }">&ndash;%&gt;--%>
            <%--&lt;%&ndash;<li class="divider"></li>&ndash;%&gt;--%>
            <%--&lt;%&ndash;<c:forEach items="${imports }" var="e">&ndash;%&gt;--%>
            <%--&lt;%&ndash;<li><a href="${ctx}/${appid }/${modelName}/importxls?xlsid=${e.ieid}"&ndash;%&gt;--%>
            <%--&lt;%&ndash;data-toggle="dialog" data-width="500" data-height="200"&ndash;%&gt;--%>
            <%--&lt;%&ndash;data-id="dialog-normal" class="green">${e.iename }-导入</a></li>&ndash;%&gt;--%>
            <%--&lt;%&ndash;</c:forEach>&ndash;%&gt;--%>
            <%--&lt;%&ndash;</c:if>&ndash;%&gt;--%>
            <%--&lt;%&ndash;<li><a href="book1.xlsx" data-toggle="doexportchecked" data-confirm-msg="确定要导出选中项吗？" data-idname="expids" data-group="ids">导出<span style="color: red;">选中</span></a></li>&ndash;%&gt;--%>
            <%--&lt;%&ndash;<li class="divider"></li>&ndash;%&gt;--%>
                ...
                <header class="panel-heading title-color">
                    ...
                    <div class="underline-nav" data-id="${record.id}-${record.name}" data-pid="${record.id}">
                        <ul class="underline-tab">
                            <li><a href="#" data-nav="doc1"
                                   data-url="${ctx}/${appid}/docmanage/doc1?projectId=${record.id}">公共档案</a></li>
                            <li><a href="#" data-nav="doc2"
                                   data-url="${ctx}/${appid}/docmanage/doc2?projectId=${record.id}">投运前档案</a></li>
                            <li><a href="#" data-nav="doc3"
                                   data-url="${ctx}/${appid}/docmanage/doc3?bdzid=${record.bdzid}">设备档案</a></li>
...
//        $tab.data("options", options);
        ...
        $.CurrentNavtab.find('.underline-nav').each(function (i, x) {
